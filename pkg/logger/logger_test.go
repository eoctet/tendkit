...
func TestLoggerContract(t *testing.T) {
	t.Run("logger-filters-levels-and-never-creates-audit-log", func(t *testing.T) {
		directory := t.TempDir()
	...
	t.Run("logger-standard-level-interfaces-own-level-dispatch", func(t *testing.T) {
		directory := t.TempDir()
	...
	t.Run("logger-leaves-historical-audit-log-untouched", func(t *testing.T) {
		directory := t.TempDir()
	...
	t.Run("logger-operation-uses-same-filter-and-writes-jsonl", func(t *testing.T) {
		directory := t.TempDir()
	...
	t.Run("logger-redacts-operation-text-and-external-structured-fields", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("logger-never-redacts-fixed-protocol-fields", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("logger-rotates-by-local-date-and-keeps-five-files", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("logger-rotates-at-size-limit-and-retains-at-most-five-files", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("logger-rejects-unsafe-active-target-before-date-rotation", func(t *testing.T) {
		directory := t.TempDir()
	...
	t.Run("logger-rotation-makes-legacy-active-and-archive-private", func(t *testing.T) {
		directory := t.TempDir()
		path := filepath.Join(directory, runLogFile)
	...
	t.Run("logger-rejects-unsafe-directory-and-destination", func(t *testing.T) {
	...
	t.Run("logger-helpers-handle-nil-and-environment", func(t *testing.T) {
		var log *Logger
	...
	t.Run("operation-output-writer-persists-one-complete-command-record", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("operation-output-writer-redacts-secret-across-write-boundaries", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("operation-output-writer-streams-json-escaping-across-utf8-boundaries", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
	...
	t.Run("operation-output-writer-bounds-pending-data-after-write-failure", func(t *testing.T) {
	...
	t.Run("new-logger-removes-stale-command-output-files", func(t *testing.T) {
		directory := t.TempDir()
	...
	t.Run("operation-output-writer-discards-when-sensitive-carry-would-exceed-bound", func(t *testing.T) {
		directory := t.TempDir()
		log, err := NewLogger(directory)
		if err != nil {
			t.Fatal(err)
