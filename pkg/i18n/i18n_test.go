...
func TestErrorTextMapsSharedStableErrors(t *testing.T) {
	previous := Current()
...
func TestDetectUsesLocalePrecedence(t *testing.T) {
...
func TestBannerIsEmbeddedWithoutTrailingLineBreak(t *testing.T) {
	banner := Banner()
...
func TestCatalogFormatsHaveMatchingArguments(t *testing.T) {
...
func TestConfigurationTerminologyIsConsistentAcrossLanguages(t *testing.T) {
...
func TestTUIMessageShortcutNamesUseUppercasePresentation(t *testing.T) {
...
func TestTranslationUsesSelectedLanguage(t *testing.T) {
	previous := Current()
	t.Cleanup(func() { Set(previous) })
	Set(Chinese)
...
func TestLocalizePreviouslyRenderedMessage(t *testing.T) {
	previous := Current()
	t.Cleanup(func() { Set(previous) })
	Set(English)
...
func TestLocalizeLeavesExternalTextUnchanged(t *testing.T) {
	previous := Current()
	t.Cleanup(func() { Set(previous) })
	Set(English)
	const message = "upstream application returned a custom error"
	if actual := Localize(message); actual != message {
		t.Fatalf("Localize(%q) = %q", message, actual)
	}
}

func TestLocalizeLeavesAmbiguousRenderedMessagesUnchanged(t *testing.T) {
	previous := Current()
	t.Cleanup(func() { Set(previous) })
	Set(English)
	...
	const message = "扫描管理"
	if actual := Localize(message); actual != message {
		t.Fatalf("Localize(%q) = %q, want original ambiguous message", message, actual)
	}
}
