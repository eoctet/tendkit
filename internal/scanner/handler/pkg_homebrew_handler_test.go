...
func TestHomebrewHandlerContract(t *testing.T) {
	t.Run("homebrew-cask-is-not-applicable-off-darwin", func(t *testing.T) {
	...
	t.Run("homebrew-formula-uses-fast-list-cellar-receipts-and-keg-walk", func(t *testing.T) {
		cellar := t.TempDir()
	...
	t.Run("homebrew-formula-dependency-receipt-is-skipped", func(t *testing.T) {
		cellar := t.TempDir()
	...
	t.Run("homebrew-formula-invalid-active-version-fails-closed", func(t *testing.T) {
		tests := []struct {
			name      string
			inventory string
		}{
	...
	t.Run("homebrew-formula-fast-inventory-requires-formulae-array", func(t *testing.T) {
		tests := []struct {
			name      string
			inventory string
		}{
	...
	t.Run("homebrew-formula-invalid-secondary-version-fails-closed", func(t *testing.T) {
		tests := []struct {
			name, field, version string
		}{
			{name: "optlinked/not installed", field: "optlinked_version", version: "2.0.0"},
			{name: "optlinked/invalid component", field: "optlinked_version", version: "../1.0.0"},
			{name: "pinned/not installed", field: "pinned_version", version: "2.0.0"},
			{name: "pinned/invalid component", field: "pinned_version", version: "../1.0.0"},
		}
		for _, test := range tests {
			cellar := t.TempDir()
			...
			inventory := `{"formulae":[{"name":"sample","versions":["1.0.0"],"linked_version":"1.0.0","` + test.field + `":` + strconv.Quote(test.version) + `}],"casks":[]}`
			runner := homebrewFormulaTestRunner(inventory, cellar)

			result := homebrewFormulaTestHandler(runner).Scan(context.Background(), Request{})

			if result.Complete || result.Err == nil || len(result.Candidates) != 0 {
				t.Errorf("%s: invalid %s result=%#v", test.name, test.field, result)
			}
		}
	})
	t.Run("homebrew-formula-invalid-receipt-fails-closed", func(t *testing.T) {
		tests := []struct {
			name    string
			receipt string
		}{
		...
		for _, test := range tests {
			...
			if result.Complete || result.Err == nil || len(result.Candidates) != 0 {
				t.Errorf("%s: invalid receipt result=%#v", test.name, result)
			}
		}
	})
	t.Run("homebrew-formula-executable-symlink-outside-keg-does-not-form-evidence", func(t *testing.T) {
		cellar := t.TempDir()
		prefix := filepath.Join(cellar, "sample", "1.0.0")
		writeHomebrewFormulaReceipt(t, prefix, true, "homebrew/core")
	...
	t.Run("homebrew-formula-directory-symlink-is-not-followed", func(t *testing.T) {
		cellar := t.TempDir()
		prefix := filepath.Join(cellar, "sample", "1.0.0")
		writeHomebrewFormulaReceipt(t, prefix, true, "homebrew/core")
	...
	t.Run("homebrew-formula-filesystem-symlink-escape-fails-closed", func(t *testing.T) {
		tests := []struct {
			name  string
	...
	t.Run("homebrew-formula-late-failure-discards-earlier-candidate", func(t *testing.T) {
		cellar := t.TempDir()
	...
	t.Run("homebrew-formula-cancellation-after-runner-fails-closed", func(t *testing.T) {
		cellar := t.TempDir()
		prefix := filepath.Join(cellar, "sample", "1.0.0")
		writeHomebrewFormulaExecutable(t, prefix, "sample")
		writeHomebrewFormulaReceipt(t, prefix, true, "homebrew/core")
	...
	t.Run("homebrew-formula-cancellation-during-walk-fails-closed", func(t *testing.T) {
		cellar := t.TempDir()
		prefix := filepath.Join(cellar, "sample", "1.0.0")
	...
	t.Run("homebrew-formula-third-party-tap-uses-canonical-name", func(t *testing.T) {
		cellar := t.TempDir()
		prefix := filepath.Join(cellar, "sample", "1.0.0")
	...
	t.Run("homebrew-cask-multiple-applications-remain-complete-and-do-not-block-other-casks", func(t *testing.T) {
		root := t.TempDir()
