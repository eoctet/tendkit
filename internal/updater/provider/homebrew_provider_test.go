...
func TestHomebrewProviderFlow(t *testing.T) {
	t.Run("homebrew-package-parser-rejects-ambiguous-or-empty-names", func(t *testing.T) {
	...
	t.Run("manager-path-rejects-non-basename-executable", func(t *testing.T) {
	...
	t.Run("homebrew-capability-matrix", func(t *testing.T) {
	...
	t.Run("homebrew-current-uses-fast-inventory-without-info", func(t *testing.T) {
		root := t.TempDir()
	...
	t.Run("registry-keeps-homebrew-current-implementation", func(t *testing.T) {
		registry := NewRegistry()
		if err := RegisterBuiltins(registry, nil, nil); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("homebrew-success-command-matrix-uses-one-absolute-manager", func(t *testing.T) {
		root := t.TempDir()
	...
	t.Run("homebrew-runner-failure-matrix-returns-typed-errors", func(t *testing.T) {
		root := t.TempDir()
		formulaRoot := filepath.Join(root, "Cellar")
		formulaPath := filepath.Join(formulaRoot, "sample", "1.0.0", "bin", "sample")
		if err := os.MkdirAll(filepath.Dir(formulaPath), 0o755); err != nil {
	...
	t.Run("homebrew-cask-is-unavailable-off-darwin-before-lookup-or-command", func(t *testing.T) {
		for _, test := range []struct {
			name       string
			capability Capability
	...
	t.Run("homebrew-update-preflight-failures-never-run-upgrade", func(t *testing.T) {
		root := t.TempDir()
	...
	t.Run("homebrew-cask-outdated-uses-dedicated-schema", func(t *testing.T) {
	...
	t.Run("verify-cask-ownership-path-accepts-caskroom-app-symlink", func(t *testing.T) {
		root := t.TempDir()
	...
	t.Run("homebrew-cask-outdated-nonzero-without-target-fails", func(t *testing.T) {
		root := t.TempDir()
